...
sidebarLayout(
  ...
  sidebarPanel(
    ...
    div(id = 'hide_date_stuff',
        ...
        tags$p('The date range should be within the dates in the input file'),
    ...
    hidden(div(id = 'hide_shit',
               ...
               tags$hr())),
    hidden(div(id = 'hide_output_select',
  ...
  mainPanel(
    ...
    tabsetPanel(id = 'main_tabset',
      tabPanel(title = 'Data', value = 'data_panel',
               ...
               hidden(h3(id = "data_table_head", "Final dataset")),
               tags$hr(),
               ...
               div(id= 'hide_text_remove', verbatimTextOutput(outputId = 'show_remove'),
                   hidden(helpText(id = 'help_text', "The table updates will be visible after confirming the data"))),
               hidden(h3(id = 'cluster_table_head', "Distribution of the clusters")),
               tags$hr(),
               tableOutput(outputId = 'cluster_table')),
      tabPanel(title = 'Graphs', value = 'graph_panel',
               h3("Main equity graph"),
               plotlyOutput(outputId = 'equity_graph'),
               tags$hr(),
               h3("Single equity graphs"), 
               tags$hr(),
               fluidRow(
                 column(3, 
      ...
      tabPanel(title = 'Sub-cluster', value = 'sub_cluster_panel',
               h3("Subcluster distribution"),
               tags$hr(),
               tableOutput(outputId = 'subcluster_table'),
               tags$hr(),
               h3("Main equity graph of subclusters"),
               plotlyOutput(outputId = 'sub_graph_equity'),
               tags$hr(),
               h3("Single equity graphs of subclusters"),
               tags$hr(),
               fluidRow(
                 column(3, 
      ...
      tabPanel(title = 'Output', value = 'output_tab_panel', 
               ...
               tags$hr(),
               h3("Cumulative equity graph of the clusters"),
               plotlyOutput(outputId = 'final_output_graph'),
               tags$hr(),
               h3("Single equity graphs of the clusters"),
...
server <- function(input, output, session){
  ...
  gen_seq <- observeEvent(input$confirm_range, {
    ...
    start = input$select_date[1]
    
      if(wday(start) < 6){
        while(wday(start) != 6){
          start = start + 1
        }
      } else if(wday(start) > 6){
        while(wday(start) != 6){
          start = start - 1
        }
      }
    
    end = input$select_date[2]
    ...
    # Removing market holidays on fridays
    master_seq = master_seq[master_seq != '2019-04-19' & master_seq != '2018-03-30']
 ...
 output$initial_content <- renderTable({
   ...
   if(file.exists('data_file.csv')){
     master = read.csv('data_file.csv', stringsAsFactors = FALSE)
     master = as.Date(master[,1])
   }
   ...
   if (is.null(input$upload)){
      return(NULL)
       ...
       data = df[df[, 1] %in% master, ]
       data[, 1] = as.character(master)
 ...
 finalise <- observeEvent(input$finalize, {
   ...
   show(id = 'data_table_head')
   hide(id = 'hide_input_row')
   ...
   mask = as.logical(apply(df, 2, remove_zeros))
   df = df[, !mask]
 ...
 # Running the clustering function when the button clicks
  cluster_df <- observeEvent(input$submit_init, {
    ...
    colnames(table) = paste("Main cluster", colnames(table), sep = " ")
    
    show(id = 'cluster_table_head', animType = "fade", anim = TRUE, time = 0.6)
  ...
  subclusters <- observeEvent(input$subcluster_init, {
    ...
    colnames(sub_table) = paste("Sub cluster", colnames(sub_table), sep = " ")
  ...
  output_tab <- observeEvent(input$output_init, {
    ...
    if(!is.null(input$select_output)){
    ...
    if (!exists('final_output_main')){
      dishun = final_output_sub
    } else if (exists('final_full_cluster')){
    ...
    output$seperate_equity_graph = renderPlotly({
      output_seperate_graph(cluster_file = df2, data_file = df1)
    })
